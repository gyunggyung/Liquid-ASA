"""Generate ASA_Qwen05B_Reproduction.ipynb with Alpaca dynamic filtering (same as LFM2.5)."""
import json
...
# ── Header ──
md([
    "# 🧪 ASA Paper Reproduction: Qwen2.5-0.5B-Instruct\n",
    "**Reproducing Table 1 from [arXiv 2602.04935](https://arxiv.org/abs/2602.04935)**\n",
    "\n",
    "Target: L*=18, AUC=0.9994, Shuffle AUC=0.4982\n",
    "\n",
    "**Same Alpaca filtering pipeline as LFM2.5 notebook** for fair comparison.\n",
    "\n",
    "⚡ T4 GPU, ~30 min total"
])
...
code([
    ...
    "CKPT = OUT/'ckpt_qwen05b_v2'; CKPT.mkdir(exist_ok=True)\n",
    "\n",
    "TOOL_S = '<functioncall>'\n",
    "TOOL_E = '</functioncall>'\n",
    "\n",
...
# ── 2. Data Pipeline (SAME AS LFM2.5) ──
md([
    "## 2 · Data Pipeline (Alpaca Filtering)\n",
    "**Identical to LFM2.5 notebook**: Download Alpaca → keyword filtering → balanced splits.\n",
    "\n",
...
code([
    ...
    "\n",
    "# ── Domain classification + tool-necessity labeling (SAME AS LFM2.5) ──\n",
...
# ── 4. Load Model ──
md(["## 4 · Load Model\n","Qwen2.5-0.5B-Instruct: 24 layers, ~494M params, pure Transformer."])
code([
    ...
    "assert NUM_LAYERS == 24, f'Expected 24 layers, got {NUM_LAYERS}'"
])

# ── 5. Hidden State Extraction ──
md(["## 5 · Hidden State Extraction\n","Last-token hidden states at all 24 layers. Checkpointed."])
code([
...
# ── 6. Probe Sweep ──
md(["## 6 · Probe Sweep → L*\n","Paper target: L*=18, AUC=0.9994. Also compute Shuffle AUC."])
code([
    "aucs = {}\n",
    "for l in LAYERS:\n",
    "    sc = StandardScaler()\n",
    ...
    "print(f'\\n🏆 L* = {L_STAR} (AUC={aucs[L_STAR]:.4f})')\n",
    "print(f'   Paper target: L*=18, AUC=0.9994')\n",
    "\n",
    "# Shuffle AUC\n",
    "N_SHUFFLE = 100\n",
    "sc_s = StandardScaler()\n",
    ...
    "print(f'   Paper: 0.4982 [0.4891, 0.5076]')\n",
    "\n",
    ...
    "ax.set_title(f'Probe Sweep: Qwen2.5-0.5B (Paper: L*=18, AUC=0.9994)')\n",
    "ax.legend(); ax.set_xticks(LAYERS); plt.tight_layout()\n",
    "plt.savefig(OUT/'probe_sweep_qwen05b.png', dpi=150); plt.show()"
])
...
code([
    ...
    "        print(f'  ⚠️ Probe {d}: skip'); continue\n",
...
# ── 9. HP Tuning ──
md(["## 9 · Hyperparameter Tuning (VALID)\n","α sweep → τ grid → β sweep. Same protocol as LFM2.5."])
code([
...
code([
    ...
    "print(f'Config: α={ALPHA}, τ={TAU}, β={BETA}, L*={L_STAR}')\n",
    "bl_res, asa_res = [], []\n",
    ...
    "ax.set_title('Baseline vs ASA: Qwen2.5-0.5B'); ax.legend(); ax.set_ylim(0,1.05)\n",
...
code([
    ...
    "\n",
    "print('\\n' + '='*70)\n",
    "print('  FINAL REPORT: Qwen2.5-0.5B-Instruct')\n",
    "print('='*70)\n",
    "print(f'L*={L_STAR}, AUC={aucs[L_STAR]:.4f} (paper: 18, 0.9994)')\n",
    "print(f'Shuffle AUC={shuf_mean:.4f} (paper: 0.4982)')\n",
    ...
    "print(f'Config: α={ALPHA}, τ={TAU}, β={BETA}')\n",
    "print('='*70)"
])
...
out_path = "ASA_Qwen05B_Reproduction.ipynb"
